...
#ifdef HAS_DIRECTIONALLIGHT
struct DirectionalLight{
	vec3 direction;
	vec3 color;
};
layout(std140) uniform DLs{
    DirectionalLight[] lights;
};
#endif
#ifdef HAS_POINTLIGHT
struct PointLight{
	vec3 position;
	vec3 color;
	vec3 attenuation;
};
layout(std140) uniform PLs{
    PointLight[] lights;
};
#endif
#ifdef HAS_SPOTLIGHT
...
layout(std140) uniform IBLs{
    ImageBasedLight[] lights;
};
#endif
